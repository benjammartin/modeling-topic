...
const CommandInput: FC = () => {
  const { URLStructure, setURLStructure, search, setSearch } =
  ...
  useKeypress('ArrowRight', () => {
    const nextIndex = (selected + 1) % URLStructure.length;
    selectElement(nextIndex);
  });

  useKeypress('ArrowLeft', () => {
    const nextIndex =
      (selected - 1 + URLStructure.length) % URLStructure.length;
    selectElement(nextIndex);
  });

  useKeypress('Backspace', () => {
    if (!search.length) {
      const newURLStructure = [...URLStructure];
      newURLStructure.pop();
      setURLStructure(newURLStructure);
    }
  });
...
const CommandList: FC<{
  ...
  const { setURLStructure, search, setSearch, setOpenModal, openModal } =
    useContext(CommandContext);
    ...
    <Fragment>
      {filteredItems.map((p, id: number) => {
        ...
        return (
          ...
          >
            <Box>
              <span className={styles.commandListItemContent}>{p.value}</span>
            </Box>
          </CommandListItem>
        );
      })}
      {filteredItems.length === 0 && <> No value</>}
    </Fragment>
  );
};
